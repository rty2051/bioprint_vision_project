...
IMAGE_PATH = "images\A5C5UniformScaffold3\DETECTED2.jpg"
...
def object_detection(image_path=IMAGE_PATH):
...
#   TEST_IMAGE = "/home/ryam/Desktop/bioprint_vision_project/images/better_square_photos/better_cam_1.jpg"
  ...
  # Find green cluster
  hues = centers[:, 0]
  green_cluster = np.argmin(np.abs(hues - 60))

  mask = (labels.flatten() == green_cluster).astype(np.uint8)
  mask = mask.reshape(hsv.shape[:2]) * 255
...
if __name__ == "__main__":
    ...
    cv2.drawContours(image, contours[1:], -1, (0, 0, 255), 2)
