...
Usage
-----
# Basic detection for Prusa bioprinter
...
python object_detection.py
    ...
    --calibrate path/to/chessboard_images/
...
def object_detection(
    ...
    @params:
        ...
        output_dir:      Directory where diagnostic images will be saved.
        min_saturation:  Minimum saturation for the pre-filter.
        h_std_factor:    Std-dev multiplier for Hue.
        s_std_factor:    Std-dev multiplier for Saturation.
        v_std_factor:    Std-dev multiplier for Value.
    ...
    @returns:
        The list of contours found (same order as cv2.findContours) and the contour image.
    """
...
def build_parser() -> argparse.ArgumentParser:
    """
    Build the command-line argument parser for the detection script.

    @returns:
        An argparse.ArgumentParser instance with all expected arguments defined.
    """
    ...
    parser.add_argument(
        "--calibrate",  "-cal",
        default=False,
        metavar="CHESSBOARD-DIRECTORY",
        help="Create a new calibration file from chessboard images."
    )
...
def main() -> None:
    ...
    if args.calibrate:
        ...
        calibration_data = run_calibration(
            image_folder    = "calibration/chessboard_prusa",
            output_file     = "calibration/calibration_data.npz",
            chessboard_size = (6, 4),
            square_size     = 13.0,
        )
        print("[INFO] Calibration complete. Results saved to 'calibration_data.npz'.\n")
        args.calibration = "calibration/calibration_data.npz"  # use new calibration for measurement

    # Object Detection
    contours, contour_image = object_detection(
    ...
    measured_contours = contours[1:]  # skip the outermost contour 
    ...
    # Label contours with mm measurements
    image = contour_image.copy() 
